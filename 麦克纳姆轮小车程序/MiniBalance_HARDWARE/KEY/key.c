...
u8  select(void) //需添加一个灰度巡线模式
{
	...
	if(count<=Angle)								       Flag_Way=0,Flag_Stop=0;  //默认模式
	else if(count>1*Angle&&count<=2*Angle) Flag_Way=2,Flag_Stop=1;	//CCD巡线模式
	else if(count>2*Angle&&count<=3*Angle) Flag_Way=3,Flag_Stop=1;	//电磁巡线模式
	else if(count>3*Angle&&count<=4*Angle) Flag_Way=4,Flag_Stop=1;	//自改巡线模式
	else TIM5->CNT=0;
